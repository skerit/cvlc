...
Cvlc.setMethod(function play(optional_file, callback) {
	...
	Fn.series(function getInstance(next) {
		...
		// just continue on playing
		if (that.play_mode == that.next_mode && that.playcount == 0) {
			return next();
		}
		...
		if (that.play_mode == 'file') {
			...
			onData = function onData(data) {
				that.proc.stdin.write(data);
			};

			// Don't pipe the stream (that would close stdin)
			that.file_stream.on('data', onData);

			// When it ends, remove the listened
			that.file_stream.on('end', function onEnd() {
				that.file_stream.removeListener('data', onData);
			});

			if (that.debug) console.log('Resuming stream');
